"""Append-only markdown decision log for Tradeyuk."""

from typing import List, Optional
from pathlib import Path
import re

from tradeyuk.agents.utils.rating import parse_rating
...
class TradingMemoryLog:
    ...
    _DECISION_RE = re.compile(r"KEPUTUSAN:\n(.*?)(?=\nREFLEKSI:|\Z)", re.DOTALL)
    _REFLECTION_RE = re.compile(r"REFLEKSI:\n(.*?)$", re.DOTALL)
    ...
    def get_past_context(self, ticker: str, n_same: int = 5, n_cross: int = 3) -> str:
        ...
        parts = []
        if same:
            parts.append(f"Analisis sebelumnya dari {ticker} (terbaru lebih dulu):")
            parts.extend(self._format_full(e) for e in same)
        if cross:
            parts.append("Pelajaran dari ticker lain baru-baru ini:")
    ...
    def _format_full(self, e: dict) -> str:
        ...
        parts = [tag, f"KEPUTUSAN:\n{e['decision']}"]
        if e["reflection"]:
            parts.append(f"REFLEKSI:\n{e['reflection']}")
        return "\n\n".join(parts)
